...
\begin{frame}[allowframebreaks]{Design of \linda{} WS}
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            %
            \begin{center}
                \texttt{template=$urlEncode(\text{\{"value": "<regex templeate>"\}})$}
            \end{center}
            %
            (notice that values are URL-encoded JSON/YAML strings)
...
\subsection{A WS for \linda{}} 

\begin{frame}[allowframebreaks]
\frametitle{Exercise \currentExercise{} -- A WS for \linda{}}
    ...
    \begin{itemize}
        ...
        \item Complete the implementation of the \linda{} WS from \texttt{sd.lab.ws.*}        
        
        \bigskip

        \item \ldots in order to adhere to the API described in:
        %
        \begin{center}
            \url{https://app.swaggerhub.com/apis/PIKA-lab/Linda-WS/2}
        \end{center}
        ...
        \item \ldots in such a way that the \alert{unit tests} in \alert{\texttt{TestRemoteTextualSpace}} class are satisfied
        %
        \begin{itemize}
            \item notice that for these tests to pass it is necessary to complete the next exercise
        \end{itemize}

        \bigskip
        
        \item Notice that implementing the WS requires taking care of:
        %
        \begin{itemize}
            \item serialisers and deserialisers in \texttt{sd.lab.ws.presentation.*}
            \item \texttt{TextualSpaceApi} in  \texttt{sd.lab.ws.tuplespaces.*}
            \item \texttt{TextualSpaceController} in  \texttt{sd.lab.ws.tuplespaces.*}
            \item \texttt{TextualSpaceStorage} in  \texttt{sd.lab.ws.tuplespaces.*}
        \end{itemize}
    \end{itemize}
\end{frame}
...
\subsection{Remote Textual Spaces} 

\begin{frame}[allowframebreaks]
\frametitle{Exercise \currentExercise{} -- Remote Textual Spaces}
    ...
    \begin{itemize}
        ...
        \item Complete the implementation of the \texttt{sd.lab.linda.textual.impl.\alert{RemoteTextualSpace}}     
        
        \bigskip

        \item which makes remote textual spaces hosted by the WS usable as \emph{ordinary} \texttt{TextualSpaces}
        %
        \begin{itemize}
            \item in a \emph{transparent} way
        \end{itemize}
        ...
        %
        \begin{itemize}
            \item notice that for these tests to pass it is necessary to complete the previous exercise
        \end{itemize}

        \bigskip
        
        \item Notice that implementing the \texttt{RemoteTextualSpace} essentially requires
        %
        \begin{itemize}
            \item wrapping an HTTP client 
            \item exploiting the client to send \emph{well formed} requests towards the WS
            \item converting responses back into \texttt{StringTuple}s or \texttt{RegexTemplate}s 
        \end{itemize}

        \bigskip

        \item Recall that distribution is just hidden: it does not disappear
        %
        \begin{itemize}
            \item many sorts of faults may be caused by distribution
            \item on the client side you should handle distribution-related issues via \texttt{sd.lab.linda.core.\alert{RemoteException}}s
        \end{itemize} 
